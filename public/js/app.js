...
var publicKey = document.getElementById("mercado-pago-public-key").value;
var mercadopago = new MercadoPago(publicKey);
function loadCardForm() {
  var productCost = document.getElementById('amount').value;
  var productDescription = document.getElementById('product-description').innerText;
  var payButton = document.getElementById("form-checkout__submit");
  var validationErrorMessages = document.getElementById('validation-error-messages');
  var form = {
    id: "form-checkout",
    cardholderName: {
  ...
  var cardForm = mercadopago.cardForm({
    ...
    callbacks: {
      ...
      onSubmit: function onSubmit(event) {
        ...
        var _cardForm$getCardForm = cardForm.getCardFormData(),
          paymentMethodId = _cardForm$getCardForm.paymentMethodId,
          issuerId = _cardForm$getCardForm.issuerId,
          email = _cardForm$getCardForm.cardholderEmail,
          amount = _cardForm$getCardForm.amount,
          token = _cardForm$getCardForm.token,
          installments = _cardForm$getCardForm.installments,
          identificationNumber = _cardForm$getCardForm.identificationNumber,
          identificationType = _cardForm$getCardForm.identificationType;
      ...
      onCardTokenReceived: function onCardTokenReceived(errorData, token) {
        if (errorData && errorData.error.fieldErrors.length !== 0) {
          errorData.error.fieldErrors.forEach(function (errorMessage) {
            alert(errorMessage);
          });
        }
        return token;
      },
      onValidityChange: function onValidityChange(error, field) {
        var input = document.getElementById(form[field].id);
...
function removeFieldErrorMessages(input, validationErrorMessages) {
  Array.from(validationErrorMessages.children).forEach(function (child) {
    var shouldRemoveChild = child.id.includes(input.id);
...
function addFieldErrorMessages(input, validationErrorMessages, error) {
  if (error) {
    ...
    error.forEach(function (e, index) {
      var p = document.createElement('p');
      p.id = "".concat(input.id, "-").concat(index);
...
function updatePrice() {
  ...
  document.getElementById('summary-description').innerText = description;
  document.getElementById('summary-total').innerText = 'R$ ' + amount;
  document.getElementById('amount').value = amount;
  document.getElementById('description').value = description;
  document.getElementById('amountTicket').value = amount;
  document.getElementById('descriptionTicket').value = description;
}
;
